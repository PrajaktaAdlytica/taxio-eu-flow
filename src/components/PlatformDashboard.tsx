...
export function PlatformDashboard() {
  return (
    <div className="surface-card overflow-hidden shadow-[0_30px_80px_-40px_rgba(11,18,32,0.25)]">
      <div className="flex items-center justify-between gap-3 border-b border-amber-200 bg-amber-50 px-4 py-2 text-[10px] font-medium text-amber-950">
        <span>Fictional demo workspace</span>
        <span className="text-amber-800/70">Controls are illustrative</span>
      </div>
      {/* Tabs */}
      ...
      <div className="flex items-center justify-between gap-3 border-b border-border px-4 py-3">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          <div className="relative flex-1 max-w-xs">
            ...
            <input
              readOnly
              aria-label="Search invoices in illustrative workspace"
          ...
          <button
            disabled
            className="h-9 px-3 rounded-md border border-border bg-background text-xs text-primary/60 disabled:cursor-not-allowed"
          >
            All Countries
          </button>
          <button
            disabled
            className="h-9 px-3 rounded-md border border-border bg-background text-xs text-primary/60 inline-flex items-center gap-1.5 disabled:cursor-not-allowed"
          >
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            disabled
            className="h-9 px-3 rounded-md bg-accent/75 text-accent-foreground text-xs font-medium inline-flex items-center gap-1.5 disabled:cursor-not-allowed"
          >
      ...
      <div className="grid lg:grid-cols-[1fr_260px]">
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="text-left text-muted-foreground border-b border-border">
                {["Invoice", "Vendor", "Country", "Amount", "VAT", "Evidence", "Status"].map(
                  (h) => (
            ...
            <tbody>
              {rows.map((r, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 tabular-nums text-primary/80 whitespace-nowrap">
                    {r.amount}
                  </td>
                  <td className="px-4 py-3 text-primary/70">{r.vat}</td>
                  <td className="px-4 py-3">
                    <Badge s={r.ev as Status} />
                  </td>
                  <td className="px-4 py-3">
                    <Badge s={r.st as Status} />
                  </td>
                </motion.tr>
              ))}
            </tbody>
        ...
        <div className="border-t lg:border-t-0 lg:border-l border-border p-4 space-y-4 bg-secondary/20">
          <div>
            ...
            <div className="mt-2 h-28 text-accent">
...
function Badge({ s }: { s: Status }) {
  ...
  return (
    <span
      className={`inline-flex items-center rounded-md px-2 py-0.5 text-[11px] font-medium ${badge[s]}`}
    >
      {label}
    </span>
  );
